...
function PackageMembers({ params }: { params: { packageId: string } }) {
  ...
  const { data: members, isLoading, refetch } = trpc.getPackageMembers.useQuery({ packageId: parseInt(params.packageId) })
  ...
  const handleAddMember = async (e: React.FormEvent) => {
    ...
    if (newMemberEmail.trim()) {
      try {
        await addMemberMutation.mutateAsync({ packageId: parseInt(params.packageId), email: newMemberEmail.trim() })
        setNewMemberEmail('')
        refetch()
      } catch (error) {
  ...
  const handleRemoveMember = async (userId: number) => {
    try {
      await removeMemberMutation.mutateAsync({ packageId: parseInt(params.packageId), userId })
      refetch()
    } catch (error) {
  ...
  if (!packageDetails?.isMember) {
    return <div>You do not have access to manage members for this package.</div>
  }
        ...
        <ul className="space-y-2 mb-4">
          {members?.map((member) => (
            <li key={member.id} className="flex justify-between items-center bg-gray-50 p-3 rounded-md">
              ...
              {packageDetails.isOwner && packageDetails.ownerId !== member.id && (
                <button
