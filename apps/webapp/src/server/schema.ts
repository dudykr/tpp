...
export const approvalRequestStatusEnum = pgEnum('approval_request_status', ['pending', 'approved', 'rejected'])
...
export const packageMembers = pgTable('package_members', {
  id: serial('id').primaryKey(),
  ...
  return {
    packageUserUnique: uniqueIndex('package_user_unique').on(table.packageId, table.userId),
  }
})
...
export const approvalGroupMembers = pgTable('approval_group_members', {
  id: serial('id').primaryKey(),
  groupId: integer('group_id').references(() => approvalGroups.id).notNull(),
  userId: integer('user_id').references(() => users.id).notNull(),
  createdAt: timestamp('created_at').defaultNow().notNull(),
}, (table) => {
  return {
...
export const approvals = pgTable('approvals', {
  ...
  return {
    requestUserUnique: uniqueIndex('request_user_unique').on(table.requestId, table.userId),
  }
})
