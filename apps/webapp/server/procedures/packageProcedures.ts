...
export const packageProcedures = {
  ...
  createPackage: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const newPackage = await ctx.db.insert(packages).values({ ...input, ownerId: ctx.user.id }).returning()
      await ctx.db.insert(packageMembers).values({ packageId: newPackage[0].id, userId: ctx.user.id })
      return newPackage[0]
    }),
